...
@app.get("/bridge/scrcpy_screenshot")
async def get_image(device_id: str):
    if not device_id:
        return JSONResponse(status_code=200, content={"message": "No device_id provided."})
...
def start_client(device_id):
    ...
    try:
        client = Client(device=device_id, max_fps=1, max_size=900, stay_awake=True)
